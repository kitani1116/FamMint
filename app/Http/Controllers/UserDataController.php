...
class UserDataController extends UserData
{

    public function contactcomp(Request $request){
        Mail::to($request->input('email'))->send(new ContactFormMail($request->all(), 'user'));

        Mail::to('fammint.app@gmail.com')->send(new ContactFormMail($request->all(), 'admin'));

        return redirect('/contact_complete');
    }


    public function contactconf(Request $request){
        
        $request->validate([
            'name' => ['required'],
            'email' => ['required','email:filter'],
            'type' => ['required','not_in:ご選択ください'],
            'message' => ['required'],
        ],[
            'required' => '入力必須です。',
        ...
        return view('Contact.contact_conf',['inputs' => $inputs ]);
    ...
    public function conf(Request $request){
        ...
        $request->validate([
            ...
            'required' => '入力必須です。',
            'email' => '正しいメールアドレスをご入力ください。',
            'unique' => 'ご入力頂いたメールアドレスは既に登録されています',
        ...
        return view('Signup.main_conf',['inputs' => $inputs ]);
        
    }

    public function childconf(Request $request){
        if(session()->has('id')){
            $request->validate([
                        'first_name' => ['required'],
                        'email' => ['required','email:filter','unique:users'],
            ...
            return view('mainaccount.child_account_conf',['inputs' => $inputs ]);
        }else{
            return redirect('/');
        }
    ...
    public function authentication(Request $request){
        ...
        if($userData == '1'){
            ...
            return redirect('/')->with(['errorM' => $errorM]) -> withInput();
        }else{
            if($userData -> role == 0){
                session() -> forget('flg');
