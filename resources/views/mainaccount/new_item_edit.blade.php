...
<div id="title_box">
  <h1 id="back_text">Item Edit</h1>
  <p id="title_text">商品の編集</p>
</div>
...
<div id="form_box">
  ...
  @enderror
    ...
    <input type="text" id="name" name="name" size="20" value="{{$item[0] -> name}}">

    @error('content')
      <p class="error_text">{{$message}}</p>
    @enderror
    <label for="content">商品内容(100文字以内):</label>
    <textarea name="content" id="" cols="30" rows="10" >{{$item[0] -> content}}</textarea><br>

    @error('price')
      <p class="error_text">{{$message}}</p>
    @enderror
    <label for="mony">価格:</label>
    <input type="text" id="mony" name="price" value="{{$item[0] -> price}}">
